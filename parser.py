...
def build_url(league, transfer_season, league_initials, season):
    return "%s/%s/%s/wettbewerb/%s/plus/?saison_id=%d$leihe=0" % (c.BASIC_URL, league, transfer_season, league_initials, season)


def get_transfers(soup):
    for club_table_header in soup.find_all("div", {"class": "table-header"})[1:]:
...
if __name__ == "__main__":
    url = build_url(c.PREMIER_LEAGUE_NAME, c.SUMMER_TRANSFER_SEASON, c.PREMIER_LEAGUE_INITIALS, 2017)
    print(url)
    soup = fetch_page(url)
